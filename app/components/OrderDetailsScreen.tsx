        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                <View style={styles.card}>
                    ...
                    <Text style={styles.infoText}>Nama: {order.profiles?.full_name || 'Tidak ada nama'}</Text>
                    <Text style={styles.infoText}>Alamat: {order.shipping_address || 'Belum diatur'}</Text>
                    <Text style={styles.infoText}>Kurir: {order.courier || 'Belum dipilih'}</Text>
                    
                    <View style={styles.divider} />

                    <Text style={styles.sectionTitle}>Metode Pembayaran</Text>
                    <Text style={styles.infoText}>{order.payment_method || 'Belum dipilih'}</Text>
                    ...
                    {order.order_items?.map((item: any, index: number) => (
                        <View key={index} style={styles.productItem}>
                            ...
                            <Text style={styles.productDetail}>Ukuran: {item.selected_size || '-'} | Qty: {item.quantity}</Text>
                        </View>
                    ))}

                    <View style={styles.divider} />

                    <Text style={styles.totalAmount}>Total Bayar: Rp {order.total_amount?.toLocaleString('id-ID')}</Text>
...
const styles = StyleSheet.create({
    ...
    header: { flexDirection: 'row', alignItems: 'center', padding: 20, backgroundColor: '#007AFF' },
    ...
    sectionTitle: { fontWeight: 'bold', marginTop: 15, fontSize: 15, color: '#333' },
    ...
    totalAmount: { fontSize: 18, fontWeight: 'bold', color: '#333', marginTop: 10 },
    payButton: { backgroundColor: '#28a745', padding: 15, borderRadius: 8, marginTop: 20, alignItems: 'center' },
